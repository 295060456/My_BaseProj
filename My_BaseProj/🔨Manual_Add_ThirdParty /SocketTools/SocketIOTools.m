...
@interface SocketIOModel ()

@end
 
@implementation SocketIOModel
...
static SocketIOTools *static_socketTools = nil;
+(instancetype)sharedInstance{
    @synchronized(self){
        if (!static_socketTools) {
            static_socketTools = SocketIOTools.new;
        }
...
-(void)linkServerWithInfo:(id)info
    ...
    if ([NSString isNullString:self.urlStr]) {
        ...
        [self.socketIOClient on:@"connect"//链接频道
            ...
            SocketIOModel *socketIOModel = SocketIOModel.new;
