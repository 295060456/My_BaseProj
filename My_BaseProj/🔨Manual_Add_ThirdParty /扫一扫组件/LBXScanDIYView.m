...
- (void)startDeviceReadyingWithText:(NSString *)text{
    ...
    CGSize sizeRetangle = CGSizeMake(self.frame.size.width - XRetangleLeft * 2,
                                     self.frame.size.width - XRetangleLeft * 2);
...
-(UIActivityIndicatorView *)activityView{
    if (!_activityView) {
        ...
        _activityView.activityIndicatorViewStyle = UIActivityIndicatorViewStyleLarge;
...
- (void)drawScanRect{
    ...
    //非扫码区域半透明
    {
        ...
        CGContextFillRect(context, rect);
        //扫码区域右边填充
        rect = CGRectMake(XRetangleRight,
                          YMinRetangle,
                          XRetangleLeft,
                          sizeRetangle.height);
        CGContextFillRect(context, rect);
        ...
        rect = CGRectMake(0,
                          YMaxRetangle,
                          self.frame.size.width,
                          self.frame.size.height - YMaxRetangle);
        CGContextFillRect(context, rect);
        //执行绘画
    ...
    if (_viewStyle.isNeedShowRetangle){
        ...
        CGContextAddRect(context, CGRectMake(XRetangleLeft,
                                             YMinRetangle,
                                             sizeRetangle.width,
                                             sizeRetangle.height));
     ...
     _scanRetangleRect = CGRectMake(XRetangleLeft,
                                    YMinRetangle,
                                    sizeRetangle.width,
                                    sizeRetangle.height);
    //画矩形框4格外围相框角
    ...
    CGContextMoveToPoint(context,
                         leftX,
                         bottomY+linewidthAngle / 2);
    CGContextAddLineToPoint(context,
                            leftX,
                            bottomY - hAngle);
    //右上角水平线
    CGContextMoveToPoint(context,
                         rightX+linewidthAngle / 2,
                         topY);
    CGContextAddLineToPoint(context,
                            rightX - wAngle,
                            topY);
...
+ (CGRect)getScanRectWithPreView:(UIView *)view
    ...
    CGSize sizeRetangle = CGSizeMake(view.frame.size.width - XRetangleLeft * 2,
                                     view.frame.size.width - XRetangleLeft * 2);
    if (style.whRatio != 1){
        CGFloat w = sizeRetangle.width;
...
//    ref:http://www.cocoachina.com/ios/20141225/10763.html  //iOS二维码扫描,你需要注意的两件事
